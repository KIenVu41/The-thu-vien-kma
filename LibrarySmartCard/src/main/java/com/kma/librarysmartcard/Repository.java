...
public class Repository {
    ...
    public void insertUser(String id, String ten, String lop, String ngayString, String diachi) throws SQLException {
      try{
          String query = " insert into user (id, ten, lop, ngaysinh, diachi)"
        + " values (?, ?, ?, ?, ?)";
     ...
     public boolean insertPubkey(String id, String modul,String expo ) throws SQLException {
      try{
          String query = " UPDATE user SET modulus = ?, exponent = ? WHERE id = ?";
    ...
    public String getModul(String id) throws SQLException{
        String modul = null;
        PreparedStatement stmt = conn.prepareStatement("select modulus from user where id = ?");  
        stmt.setString (1, id);
    ...
    public String getExpo(String id) throws SQLException{
        ...
        PreparedStatement stmt = conn.prepareStatement("select exponent from user where id = ?");  
        stmt.setString (1, id);
        ResultSet rs=stmt.executeQuery();  
        while(rs.next()){  
